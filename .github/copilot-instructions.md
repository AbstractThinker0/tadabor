...
1. UIProvider (Chakra UI provider)
2. Toaster providers
3. ReloadPrompt (PWA)
4. Error boundaries
...
**Global Stores** (`src/store/global/`): settings, navigation, user, translations, localNotes, cloudNotes

**Page-Specific Stores** (`src/store/pages/`): Store UI state like selected chapters, search filters, scroll positions (e.g., quranBrowserPage, rootsBrowserPage).
...
- Use Zustand stores for shared UI state (e.g., `navigation.toolInspect`, `navigation.pageDirection`)
- Use Context for data sharing (e.g., `QuranContext` for Quran instance)
- Avoid prop drilling; prefer Zustand for non-local state

## Key Files to Reference

- **App Entry**: `src/index.tsx`, `src/App.tsx` (route definitions)
- **State**: `src/store/global/` (Zustand stores), `src/store/global/localNotes.ts` (notes pattern)
...
**Last Updated**: January 7, 2026 | **Version**: 0.47.5
