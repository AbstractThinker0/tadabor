...
import { dbLetters, isPresetDefinitionReference } from "@/util/dbFuncs";
  ...
  immer(
    combine(initialState, (set, get) => ({
      ...
      deletePreset: async (presetID: string) => {
        const { error } = await tryCatch(dbLetters.deletePreset(presetID));
        ...
        set((state) => {
          delete state.letterPresets[presetID];

          Object.keys(state.lettersDefinitions).forEach((defKey) => {
            if (state.lettersDefinitions[defKey]?.preset_id === presetID) {
              delete state.lettersDefinitions[defKey];
            }
          });

          Object.keys(state.lettersData).forEach((verseKey) => {
            Object.keys(state.lettersData[verseKey]).forEach((letterKey) => {
              if (isPresetDefinitionReference(state.lettersData[verseKey][letterKey].def_id, presetID)) {
                state.lettersData[verseKey][letterKey].def_id = "";
              }
            });
          });
